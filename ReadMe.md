...
```bash
npm run dev
This starts the server using nodemon, which automatically restarts the server on code changes.

On successful startup:

The server listens on port 5050

MongoDB connects successfully using the connection string from .env
...
This file is responsible only for starting the application, not defining routes or business logic.
